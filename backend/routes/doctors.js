...
router.post('/my/ai-insight', auth, doctorAuth, async (req, res) => {
    try {
        ...
        const cacheKey = `cache:ai-insight:doctor:${doctorId}:${period || '30d'}:${gender || 'all'}:${Buffer.from(kategoriFP).toString('base64').slice(0, 40)}`;
...
${trendText}`;
        ...
        const completion = await _groqDoc.chat.completions.create({
            ...
            messages: [
                { role: 'system', content: 'Kamu adalah AI Clinical Analytics Assistant untuk dashboard dokter klinik. Tugasmu adalah menganalisis statistik kunjungan pasien dan menjelaskan pola yang terlihat dari data. Jangan pernah memberikan saran medis atau operasional.' },
